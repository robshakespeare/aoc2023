...
public class Day13Solver : ISolver
{
    ...
    {
        return Grid.ParseInput(input).Sum(grid => grid.GetMirrorLine(smudge: false));
    ...
    public record Grid(string GridFull, string[] Rows)
    {
        ...
        {
            //var result = GetMirrorLine(Rows, isHorizontal: true, smudge) ?? GetMirrorLine(Columns, isHorizontal: false, smudge);
            ...
            return GetMirrorLine(Rows, isHorizontal: true, smudge) ?? GetMirrorLine(Columns, isHorizontal: false, smudge) ?? throw new Exception("Couldn't find mirror line");
        }
        ...
        static long? GetMirrorLine(string[] lines, bool isHorizontal, bool smudge)
        {
            ...
            for (var candidate = 0; candidate < lines.Length - 1; candidate++)
            {
                ...
                if (endReached && differences == targetDifferences) // && numMatches > 0)
                {
